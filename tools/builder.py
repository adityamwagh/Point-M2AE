...
def dataset_builder_modelnet40():
    train_val_loader = DataLoader(
        ModelNet40SVM(partition="train", num_points=1024), num_workers=8, batch_size=128, shuffle=True
    )
    test_val_loader = DataLoader(
        ModelNet40SVM(partition="test", num_points=1024), num_workers=8, batch_size=128, shuffle=True
    )
