...
def get_args():
    ...
    parser.add_argument(
        "--resume", action="store_true", default=False, help="autoresume training (interrupted by accident)"
    )
    parser.add_argument("--test", action="store_true", default=False, help="test for certain ckpt")
    parser.add_argument(
        "--test_svm", choices=["modelnet40", "scanobjectnn"], default=None, help="test_svm for certain ckpt"
    )
    parser.add_argument(
        "--finetune_model", action="store_true", default=False, help="finetune modelnet with pretrained weight"
    )
    parser.add_argument("--scratch_model", action="store_true", default=False, help="training modelnet from scratch")
    parser.add_argument(
        "--mode", choices=["easy", "median", "hard", None], default=None, help="difficulty mode for shapenet"
    )
    ...
    args.experiment_path = os.path.join(
        "./experiments", Path(args.config).stem, Path(args.config).parent.stem, args.exp_name
    )
    args.tfboard_path = os.path.join(
        "./experiments", Path(args.config).stem, Path(args.config).parent.stem, "TFBoard", args.exp_name
    )
