...
class PointcloudScaleAndTranslate(object):
    ...
    def __call__(self, pc):
        ...
        for i in range(bsize):
            ...
            pc[i, :, 0:3] = (
                torch.mul(pc[i, :, 0:3], torch.from_numpy(xyz1).float().cuda()) + torch.from_numpy(xyz2).float().cuda()
            )
...
class PointcloudJitter(object):
    ...
    def __call__(self, pc):
        bsize = pc.size()[0]
        for i in range(bsize):
            jittered_data = pc.new(pc.size(1), 3).normal_(mean=0.0, std=self.std).clamp_(-self.clip, self.clip)
